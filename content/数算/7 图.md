...
注意 DFS 不要求从入度为 0 的顶点开始，比如对于 $A\rightarrow B\rightarrow C$ 
...
- 不作删除旧值的缺点是，在最差情况下，将使堆中元素数目增加到 $\Theta(|V|)$ ，此时总的时间复杂度为 $\Theta((|V| + |E|)\log |E|)$ （每次取出顶点、每次更新边权均需要调整最小堆）
...
- 初始时，把每个顶点都视为一个等价类
- 按照边权从小到大的顺序处理，如果一条边连接两个不同等价类，则将边加入 MST ，并把两个等价类合并
