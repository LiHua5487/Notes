...
但这可能存在一些 outlier ，如果 outlier 参与了变换矩阵的计算，结果会有偏差，可以使用 RANSAC 进行剔除，先随机选 4 组点计算一个矩阵，据此计算每个点变换后的坐标，并计算与对应点坐标间的误差，把误差小于一定阈值的视为 inlier，而后计算 inlier 比例，选择 inlier 比例最大的情况，用这些 inlier 计算矩阵
