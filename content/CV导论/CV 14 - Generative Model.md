...
VAE是使用两个网络，一个学习把图像编码成向量，另一个学习把向量解码回图像，它们的目标是让复原图像和原图像尽可能相似,学习完毕后，解码器就是图像生成网络
扩散模型可以看作一种更具体的VAE，把编码过程固定为加噪声，并让解码器学习怎么样消除之前添加的每一步噪声，下面是一个最基本的模型 Denoising Diffusion Probabilistic Models (DDPM)
...
**反向过程**：模型的目标是学习一个“去噪模型”，从随机噪声 $x_T$ 开始，逐步去掉噪声，还原到原始数据分布 $x_0$
...
上式中 $x_t$ 对于反向过程是已知的，所以只用学习 $\boldsymbol{\epsilon}_\theta(\mathbf{x}_t, t)$ ，损失函数可简化为以下形式
