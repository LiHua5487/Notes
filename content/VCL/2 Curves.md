...
显示隐式表达
- 显示：函数方程 $y=f(x)$ ，参数化方程 $x=r \text{cos}\theta, y=r \text{sin}\theta$ 
- 隐式：隐函数方程（解析方程） $f(x,y)=0$ ，分形，SDF

连续性
- $C^n$ 连续（参数连续）：n 阶导数连续
- $G^n$ 连续（几何连续）：通过切线方向和曲率定义

![[image-0.png]]

$C^n$ 连续能推出 $G^n$ 连续，但反过来不一定（因为 $G^n$ 是针对于曲线本身形状的，但 $C^n$ 是针对于某种参数化的，不同参数化方式的连续性不一定一样）

>以开车经过弯道为比喻， $G^n$ 就好比打方向盘调整车的方向是平滑变化的，而 $C^n$ 还进一步要求开车的速度也是平滑变化的 
...
![[VCL/img/img2/image-1.png|700x226]]
...
其中标记出的四个多项式称为 **厄米特基函数 Hermite basis functions**
