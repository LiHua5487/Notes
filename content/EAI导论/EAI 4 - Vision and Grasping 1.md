...
>Singulation 分离 / 挑拣：机器人从一个杂乱堆积的物体群中，分离出目标物体的过程，它不是简单地“抓取”，而是在抓取之前或之中，通过主动的物理交互（如推、拨、拉、摇），为成功的抓取创造条件
...
那怎么获得抓取的 pose 呢
- 如果物体的形状等信息已知，标注了标准状态下的抓取 pose ，则可以直接根据物体的当前 pose 计算出抓取 pose 进行抓取，这样只需要预测物体 pose 就行了
- 对于未知物体，需要直接预测抓取 pose 

>标注抓取 pose 时，往往结合了如何使用的先验，如果直接预测抓取 pose ，可能只是单纯抓起来，但是没法使用这个物体

>相较于端到端的方式，在实际应用场景中（要抓的东西比较固定，场景一般不会特别复杂，但是对成功率要求比较高），这种方式的稳定性与可解释性往往更强一些，且便于加入规则
...
3D 旋转只有 3 个自由度，但是旋转矩阵有 9 个数字，这使得旋转很难学，考虑其它的旋转表示法，比如欧拉角、轴角、四元数，但是它们又不连续，或者在一些情况下不能与旋转矩阵一一对应
...
需要先用一个训好的模型对图片进行处理，得到 mask，预测图片中的点在不在物体上，而后通过另一个模型预测这些点对应到模型上的坐标
